...
describe("SessionManager", () => {
  ...
  describe("getOrCreate", () => {
    it("creates a new session on first call", () => {
      const session = mgr.getOrCreate("s1", "vscode", "My Label");
    ...
    it("returns existing session on repeat call", () => {
      const first = mgr.getOrCreate("s1", "vscode", "Label1");
      const second = mgr.getOrCreate("s1", "vscode", "Label1");
      expect(second).toBe(first);
    });

    it("generates default label when none provided", () => {
      const session = mgr.getOrCreate("abcdefgh-1234", "cli");
      expect(session.label).toBe("cli-abcdefgh");
    });

    it("updates label when a new one is provided", () => {
      mgr.getOrCreate("s1", "vscode", "OldLabel");
      const updated = mgr.getOrCreate("s1", "vscode", "NewLabel");
      expect(updated.label).toBe("NewLabel");
    });

    it("does not update label if same value", () => {
      const original = mgr.getOrCreate("s1", "vscode", "Same");
      const again = mgr.getOrCreate("s1", "vscode", "Same");
  ...
  describe("updateStatus", () => {
    it("updates status and touches lastActivityAt", () => {
      const session = mgr.getOrCreate("s1", "vscode");
      const originalActivity = session.lastActivityAt;

      // Force a different timestamp
      mgr.updateStatus("s1", "waiting_for_input");
    ...
    it("does nothing for unknown session", () => {
      // Should not throw
      mgr.updateStatus("nonexistent", "error");
    });
  });

  describe("get", () => {
    it("returns session by id", () => {
      mgr.getOrCreate("s1", "vscode", "Test");
  ...
  describe("listAll", () => {
    ...
    it("returns all sessions", () => {
      mgr.getOrCreate("s1", "vscode");
      mgr.getOrCreate("s2", "cli");
